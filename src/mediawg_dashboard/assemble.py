...
def build_activity(
    ...
    now: datetime,
    last_discussion: list[dict] | None = None,
) -> SpecActivity:
    """Fold the raw activity payloads into one event list (no I/O).

    The three window payloads share a single failure key, so **any** of them
    being ``None`` marks the whole axis unknown. Reporting a partial count would
    be worse than saying nothing: a comments outage would render as a confident
    "0 new".

    ``last_discussion`` is separate: it feeds the Pulse recency signal and is not
    window-limited, so it stays useful even when the window is empty.
    """
    discussion_days = (
        days_since_last_discussion(last_discussion, now=now)
        if last_discussion is not None
        else None
    )
    if updated_issues is None or comments is None or review_comments is None:
        return SpecActivity(
            window_days=ACTIVITY_WINDOW_DAYS, known=False, last_discussion_days=discussion_days
        )
    ...
    return SpecActivity(
        ...
        events=events,
        known=True,
        last_discussion_days=discussion_days,
    )
...
def build_spec(
    ...
    commits = commits or []
    health = SpecHealth(
        days_since_commit=days_since_last_commit(commits, now=now),
    ...
    return Spec(
        ...
        activity=build_activity(
            updated_issues, comments, review_comments, now, last_discussion
        ),
    )
...
def merge_spec(fresh: Spec, prev: Spec | None, failed: set[str]) -> Spec:
    ...
    if "commits" in failed:
        s.health.days_since_commit = prev.health.days_since_commit
